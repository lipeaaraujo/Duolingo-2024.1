...
| <a id="coroa">Coroa</a> | Refere-se a uma [recompensa](#recompensas) visual ou pontuação que o usuário recebe após completar com sucesso uma [lição](#licao) ou atividade no Duolingo | As [coroas](#estrela) servem como um indicador de [desempenho](#desempenho) e sucesso, motivando o [usuário](#usuario) a alcançar um bom [desempenho](#desempenho) em cada conteúdo. Elas podem ser usadas para medir o [progresso](#progresso) dentro de uma [unidade](#unidade) e incentiver o [usuário](#usuario) a manter um alto nível de acerto | Pontuação, Emblema, Selo de aprovação |
